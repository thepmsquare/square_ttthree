...
def render_board(board):
    ...
    for i in range(0, 9, 3):
        rows.append(
            f" {symbols[board[i]]} | {symbols[board[i + 1]]} | {symbols[board[i + 2]]} "
        )
    print("\n" + "\n---+---+---\n".join(rows) + "\n")
...
def run_debug_game():
    ...
    while not done:
        ...
        exploration_type = (
            "random (exploration)" if is_random else "greedy (exploitation)"
        )
        ...
        if prev_states[current_player] is not None:
            ...
            print(
                f"  q-value: {old_q:.4f} -> {new_q:.4f} (change: {new_q - old_q:+.4f})"
            )
        ...
        if done:
            ...
            print(
                f"  q-value: {active_old_q:.4f} -> {active_new_q:.4f} (change: {active_new_q - active_old_q:+.4f})"
            )
            ...
            if prev_states[opponent] is not None:
                ...
                print(
                    f"  q-value: {opp_old_q:.4f} -> {opp_new_q:.4f} (change: {opp_new_q - opp_old_q:+.4f})"
                )

    print("\nfirst debug game finished!")
    print(
        "as you play more games, these values will build up and propagate further back."
    )
