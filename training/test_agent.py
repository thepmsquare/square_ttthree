...
def evaluate_agent(difficulty: str, num_games: int = 1000):
    ...
    model_path = os.path.join(
        os.path.dirname(__file__), "models", f"ttthree_{difficulty}.json"
    )
    ...
    while not done:
        ...
        if current_player == agent_player:
            ...
            while action not in valid_moves:
                try:
                    user_input = input(
                        f"your turn! enter an empty space {valid_moves}: "
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="evaluate or play against the ttthree agents."
    )
    parser.add_argument(
        "--mode",
        type=str,
