...
def test_configuration_exception(
    get_patched_configuration, mock_config_dict, capsys, mocker
):
    ...
    mocker.patch("square_commons.ConfigReader", return_value=mock_instance)
...
def test_configuration_invalid_formatter(
    get_patched_configuration, mock_config_dict, mocker
):
    invalid_dict = mock_config_dict.copy()
    ...
    mocker.patch("square_commons.ConfigReader", return_value=mock_instance)
    with pytest.raises(ValueError, match="Invalid formatter choice: invalid_formatter"):
        importlib.reload(square_ttthree.configuration)
